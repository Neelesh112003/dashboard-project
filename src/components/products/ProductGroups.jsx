...
export default function ProductGroups() {
  ...
  async function fetchProductGroups() {
    try {
      const response = await api.get("/v1/product-groups");
      ...
      const formattedData = response.data.data.data.map((item) => ({
        ...
        sector: item.sector,
        productGroupStatus: item.product_group_status,
      }));
      ...
      showToast("Failed to fetch Product Groups");
    }
  }
  ...
  async function handleSubmit(e) {
    ...
    if (!formData.groupCode) {
      showToast("Group Code is required");
      return;
    }
    ...
    const payload = {
      ...
      product_group_status: formData.productGroupStatus,
      status: "active",
    };
    ...
    if (editingId) {
      try {
        ...
        showToast(response.data.message || "Product Group Updated");
        ...
        showToast(
          error.response?.data?.message || "Failed to update Product Group",
        );
    ...
    // CREATE
    try {
      const response = await api.post("/v1/product-groups", payload, {
        headers: {
          "Content-Type": "application/json",
        },
      });
      ...
      showToast(response.data.message || "Product Group Created");
      ...
      showToast(
        error.response?.data?.message || "Failed to create Product Group",
      );
    }
  }
  ...
  async function handleDelete(id) {
    try {
      const response = await api.delete(`/v1/product-groups/${id}`);

      console.log("Deleted:", response.data);

      showToast(response.data.message || "Product Group Deleted");
      ...
      showToast(
        error.response?.data?.message || "Failed to delete Product Group",
      );
    }
  }
  ...
  function handleEdit(row) {
    setFormData({
      ...
      sector: row.sector || "",
      productGroupStatus: row.productGroupStatus || "active",
    });
  ...
  // Apply productGroupStatus filter if one is selected
  if (filters.productGroupStatus !== "") {
    filteredList = filteredList.filter(
      (item) => item.productGroupStatus === filters.productGroupStatus,
    );
  }
      ...
      <div className="space-y-8">
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="relative">
                    ...
                    <select
                      ...
                      value={formData.productGroupStatus}
                      onChange={handleInputChange}
                      className="w-full rounded-xl border border-slate-200 bg-slate-50 py-2.5 pl-11 pr-4 text-sm outline-none"
                    >
        ...
        <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-[#162033] dark:bg-[#0d1528]">
          ...
          <div className="flex flex-wrap items-center gap-3 border-b border-slate-100 bg-slate-50 px-6 py-3">
            ...
            <select
              value={filters.productGroupStatus}
              onChange={(e) => {
                setFilters({ ...filters, productGroupStatus: e.target.value });
                setCurrentPage(1);
              }}
              className="rounded-lg border border-slate-200 bg-white px-3 py-1.5 text-xs outline-none"
            >
              <option value="">All Product Group Status</option>
              {getUniqueValues("productGroupStatus").map((val) => (
                <option key={val}>{val}</option>
              ))}
            </select>
          </div>
